		...
		describe "should default to 1x1 block",->
			...
			describe "sould default height to 1",->
				...
				Then ->
					...
					expect(@block.width).toBe 1
		describe "sould be able to set new width",->
			Given -> 
				@block = new G.l.Block {}
			When ->
				@block.setWidth(10)
			Then ->
				expect(@block.width).toBe(10)
		describe "should be able to set new height",->
			Given ->
				@block = new G.l.Block {}
			When ->
				@block.setHeight 10
	...
	describe "Breakers", ->
		describe "breakers should default to false",->
			Given ->
				@block = new G.l.Block({})
			Then ->
				expect(@block.isBreaker).toBe false
		describe "a block that is a breaker returns true to isBreaker",->
			Given ->
				@block = new G.l.Block({}).Breaker()
			Then ->
				expect(@block.isBreaker).toBe true
				expect(@block instanceof G.l.Block).toBe true
		describe " a breaker can be unbroken",->
			Given ->
				@breaker = new G.l.Block({}).Breaker();
			When ->
				@breaker.UnBreaker();
			Then ->
				expect(@breaker.isBreaker).toBe false
